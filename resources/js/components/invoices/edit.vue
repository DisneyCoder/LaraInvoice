...
const getInvoice = () => {
    fetch("/api/editInvoice/" + props.id)
        .then((response) => response.json())
        .then((data) => {
            // console.log('fetedEditedData', data.invoice)
...
const onEdit = () => {
    if (form.value.invoice_items.length > 0) {
        ...
        formData.append("discount", Number(form.value.discount) || 0);
        ...
        axios
            .post(`/api/updateInvoice/${form.value.id}`, formData)
            .then(() => {
                form.value.invoice_items = [];
                router.push("/");
            })
